...
class ResponsibleWithoutFlag extends BaseChecker {
  ...
  FunctionDefinition(node) {
    ...
    // bounce
    const hasBounce = args.identifiers.some(subNode => subNode.name === 'bounce')

    if (!hasBounce) this._warn(node, 'Default bounce is omitted in return of responsible function')

    // value
    const hasValue = args.identifiers.some(subNode => subNode.name === 'value')

    if (!hasValue) this._warn(node, 'Default value is omitted in return of responsible function')
  }
